...
function getProductFinalPrice($id,$qnt) {
	
	$sql=$GLOBALS['obj']->query("select * from tbl_product where id='$id' ");
	$result=mysqli_fetch_assoc($sql);
	//echo $result['qnt_size1'];
	if ($result['qnt_size1']!=0 && $qnt>=$result['qnt_size1'] && $qnt<$result['qnt_size2'] && $qnt<$result['qnt_size3']) {
		return $result['price_size1'];

	} else if($result['qnt_size1']!=0 && $qnt>$result['qnt_size1'] && $qnt>=$result['qnt_size2'] && $qnt<$result['qnt_size3']){
		return $result['price_size2'];
		
	}else if($result['qnt_size1']!=0 && $qnt>$result['qnt_size1'] && $qnt>$result['qnt_size2'] && $qnt>=$result['qnt_size3']){
		return $result['price_size3'];
	}else if($result['qnt_size1']==0){
...
function getTotalSales($id) {	
	$sql=$GLOBALS['obj']->query("select sum(total_amount) as total_amount from tbl_order where user_id='$id' and order_status=3",$debug=-1);
	$result=mysqli_fetch_assoc($sql);
	return ($result['total_amount']);
}
