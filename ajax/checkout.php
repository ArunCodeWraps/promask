...
if (!empty($_SESSION['sess_user_id'])) {
          ...
          $obj->query("insert into $tbl_order set $OSql,order_date='$order_date' ",$debug=-1); 
          ...
          if ($payment_method=='cod') {
            $_SESSION['mythanks']=1;
            $response=array('status' =>1,'msg'=>'Your order success','pmode'=>'cod');     

          }else if($payment_method=='online'){
            $_SESSION['mythanks']=1;
            $response=array('status' =>1,'msg'=>'Your order success','pmode'=>'online'); 

          }


 } else {
    $response=array('status' =>0,'msg'=>'Please login first, before place order.','pmode'=>'test'); 
}         
