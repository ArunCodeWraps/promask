                ...
                <div class="content-header-left col-md-12 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        ...
                        <div class="col-6" style="text-align: right;">
                            <button class="btn btn-outline-primary" onclick="document.location.href='banner-addf.php';"><span><i class="feather icon-plus"></i> Add New</span></button>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table data-list-view">
                            ...
                            <tbody>
                              <?php
                              $i=1;
                              $sql=$obj->query("select * from $tbl_banner where 1=1  order by id desc",$debug=-1);
                              while($line=$obj->fetchNextObject($sql)){?>
                                <tr>
                                    <td><input type="checkbox" class="dt-checkboxes" name="ids[]" value="<?php echo $line->id ?>"></td>
                                    <td class="product-image"><img src="../upload_image/banner/thumb/<?php echo $line->photo ?>" class="img-responsive thumb-img"></td>
                                    <td class="product-name"><?php echo stripslashes($line->title); ?></td>
                                    <td class="product-price">
                                        <p></p>
                                        <div class="custom-control custom-switch custom-switch-success mr-2 mb-1">
                                            <input type="checkbox" class="custom-control-input chkstatus" value="<?php echo $line->id;?>" <?php echo ($line->status=="1")?'checked':'' ?> data-one="<?php echo $tbl_banner?>"  id="customSwitch<?php echo $line->id ?>">
                                            <label class="custom-control-label" for="customSwitch<?php echo $line->id ?>"></label>
                                        </div>
                                    </td>
                                    <td class="product-action">
                                        <a href="banner-addf.php?id=<?php echo $line->id;?>"><i class="feather icon-edit"></i></a>
                                    </td>
                                </tr>
                                <?php $i++; }?>
