...
if(count($arr)>0){
		...
		if($Submit=='Delete')
		{
			...
			$imageArr=$obj->query("select logo from  tbl_business where id in ($str_rest_refs) ");
			while($resultImage=$obj->fetchNextObject($imageArr)){
			@unlink("../upload_image/business/".$resultImage->logo); 
			}
		    $sql="delete from tbl_business where id in ($str_rest_refs)"; 
			$obj->query($sql);
		...
		{	
			$sql="update tbl_business set status=1 where id in ($str_rest_refs)";
			$obj->query($sql);
		...
		{		
			 $sql="update tbl_business set status=0 where id in ($str_rest_refs)";
			$obj->query($sql);
