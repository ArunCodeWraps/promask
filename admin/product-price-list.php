           ...
           <div class="content-header row">
                <div class="content-header-left col-md-12 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-6">
                            <h2 class="content-header-title float-left mb-0">Manage Price/Size</h2>
                        </div>
                        <div class="col-6" style="text-align: right;">
                            <button class="btn btn-outline-primary" onclick="document.location.href='product-list.php';"><span><i class="feather icon-list"></i> Product List</span></button>

                            <button class="btn btn-outline-primary" onclick="document.location.href='product-price-addf.php?product_id=<?php echo $_REQUEST['product_id'] ?>';"><span><i class="feather icon-plus"></i> Add Price & Size</span></button>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table data-list-view">
                            ...
                            <tbody>
                              <?php
                              $i=1;
                              $sql=$obj->query("select * from $tbl_product_prices where 1=1 and product_id='".$_REQUEST['product_id']."'",$debug=-1);
                              while($line=$obj->fetchNextObject($sql)){ 
                                ...
                                <tr>
                                    ...
                                    <td class="product-image"><img src="../upload_image/product/thumb/<?php echo $line->photo ?>" class="img-responsive thumb-img"></td>
                                    <td class="product-name"><?php echo $line->size." / ".$res->name; ?></td>
                                    <td class="product-name"><?php echo $line->price; ?></td>
                                    
                                    <td class="product-price">
                                        <p></p>
                                        <div class="custom-control custom-switch custom-switch-success mr-2 mb-1">
                                            <input type="checkbox" class="custom-control-input chkstatus" value="<?php echo $line->id;?>" <?php echo ($line->status=="1")?'checked':'' ?> data-one="<?php echo $tbl_product_prices?>"  id="customSwitch<?php echo $line->id ?>">
                                            <label class="custom-control-label" for="customSwitch<?php echo $line->id ?>"></label>
                                        </div>
                                    </td>
                                    <td class="product-action">
                                       <a href="product-price-addf.php?id=<?php echo $line->id;?>&product_id=<?php echo $_REQUEST['product_id'] ?>" title="Edit"><i class="feather icon-edit"></i></a>
                                    </td>
                                </tr>
                                <?php $i++; }?>
    ...
    function del_prompt(frmobj,comb)
        ...
        //alert(comb);
            if(comb=='Delete'){
                ...
                {
                    frmobj.action = "product-price-del.php?product_id=<?php echo $_REQUEST['product_id']; ?>";
